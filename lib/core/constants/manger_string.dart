class MangerString {

  // Login Screen Text Enter required data!
  static const String textLogin = 'تسجيل الدخول';
  static const String identifyNumber = 'رقم الهوية';
  static const String password = 'كلمة المرور';
  static const String forgetPassword = 'هل نسيت كلمة المرور؟';
  static const String enterRequiredData = 'ادخل البيانات المطلوبة';
  static const String loginSuccessfully = 'تم الدخول بنجاح';

  // RESET PASSWORD SCREEN
  static const String resetPassword = 'إعادة تعيين  كلمة المرور';

  //bottomBar

  static const String homeBottomBar = 'الرئيسة';
  static const String homeScreenText = 'الصفحة الرئيسة';
  static const String myDataBottomBar = 'بياناتي';

  // MyDataScreen
  static const String nameUser = 'المحفظ : محمد ابوشملة';
  static const String identifyNumberUser = 'رقم الهوية : 462822842';
  static const String siteUser = 'المنطقة : الوسطى';
  static const String jobUser = 'الوظيفة: مشرف ميداني';
  static const String jobUser1 = 'الوظيفة: محفظ ';

  // AddStudentScreen

  static const String titleAddScreen = 'إضافة طالب ';
  static const String studentName = 'اسم الطالب';
  static const String startSave = 'بداية الحفظ';
  static const String dateStart = 'تاريخ التسجيل';

  static const String add = 'إضافة';
  static const String cancle = 'إلغاء';

  static const String settings = 'الإعدادات';
  // DrawerItem String
  static const String test1 = 'إختبارت (المحفظ)';
  static const String test2 = 'إختبارت (المشرف)';
  static const String daily = 'السجل اليومي';
  static const String monthly = 'السجل الشهري';
  static const String support = 'الدعم الفني';
  static const String logout = 'تسجيل الخروج';

  // daily log Screen
  static const String addSaveDaily = 'إضافة الحفظ اليومي';
  static const String todayDate = 'تاريخ اليوم';
  static const String save = 'الحفظ';
  static const String review = 'المراجعة';
  static const String from = 'من';
  static const String to = 'الى';

  // homeScreen

  static const String welcomeHome = 'اهلا بك : محمد ابوشملة';
  static const String numberID = '4079534673';
  static const String supervisor = 'مشرف المنطقة الوسطى';
  static const String episodeName = 'حلقة عمر بن الخطاب';


  static const String addPurchase = 'إضافة محفظ ';
  static const String phoneNumber = 'رقم الهاتف';
  static const String city = 'المحافظة';
  static const String theWorkOfTheGuardian = 'عمل ولي الامر';
  static const String saveAmount = 'مقدار الحفظ';
  static const String academicLevel = 'المستوى الاكاديمي';
  static const String socialLevel = 'المستوى الاجتماعي';
  static const String talent = 'الموهبة';


  static const String remove = 'حذف';
  static const String edite = 'تعديل';










}